...
class SouGou:
        ...
        # 获取搜狗热点
        def get_sg_toutiao(self):
                try:
                    url = 'http://top.sogou.com/hot/shishi_1.html'
                    print(url)
                    html = self.re_spider_page_data(url)
                    soup = BeautifulSoup(html, "lxml")
                    feeds_div = soup.find('ul', class_='pub-list').find_all('li')
                    flag = 0
                    for feed in feeds_div:
                        try:
                            ...
                            if flag == 0 or flag == 1 or flag == 2:
                                sg_context = feed.find('span', class_='s2').find('p', class_='p1').a.get_text().strip()
                            else:
                                sg_context = feed.find('span', class_='s2').get_text().strip()
                            # 文章链接
                            feed_url = 'https://www.sogou.com/sogou?query='+sg_context
                            # 文章发布时间和作者
                            sg_time = time_sleep.strftime('%Y-%m-%d')
                            result['文章链接'] = feed_url
                            result['文章标题'] = sg_context
                            result['文章发布时间'] = sg_time
                            result['文章内容'] = sg_context
                            # 追加到集合中

...
def parse_data(html):
    ...
    users = soup.find_all(id='table1')[0].find_all('tr')
    for user in users:
        # 返回结果
        result = collections.OrderedDict()
        tds = user.find_all("td")
        result['被执行人'] = tds[1].get_text()
        result['住所地'] = tds[2].get_text()
        result['身份证'] = tds[3].get_text()
        result['依据文书'] = tds[4].get_text()
        result['申请执行人'] = tds[5].get_text()
        result['联系人'] = tds[6].get_text()
        # 追加到集合中
...
def re_spider_page_data():
    ...
    for page in range(1, 2):
        # 获取的html数据
        url = "http://hpqfy.hbfy.gov.cn/DocManage/ViewDoc?docId=7415ac8b-2ce9-4d56-b0fb-66671f041e5b"
        ...
        # print(result)
        with open('./hbfy_gov_blacklist.json', 'a') as f:
            f.write(result)
            f.write("\n")
